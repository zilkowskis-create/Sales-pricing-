(async()=>{
  ...
  try{
    ...
    async function saveCollisionFile(file){if(!file)return;try{const db=await openDb();await new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).put({blob:file,name:file.name,type:file.type,lastModified:file.lastModified,savedAt:Date.now()},'collision');tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});db.close();}catch(e){console.warn('Collision file could not be cached',e);}}
    async function getSavedCollisionFile(){try{const db=await openDb();const rec=await new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const rq=tx.objectStore(STORE).get('collision');rq.onsuccess=()=>resolve(rq.result||null);rq.onerror=()=>reject(rq.error);});db.close();if(!rec?.blob)return null;return new File([rec.blob],rec.name||'S&GMJ Collision.xlsx',{type:rec.type||'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',lastModified:rec.lastModified||Date.now()});}catch(e){console.warn('Saved Collision file could not be restored',e);return null;}}
    async function restoreCollisionFile(){const input=document.getElementById('collisionFile');if(!input||input.files?.length)return;const file=await getSavedCollisionFile();if(!file)return;try{const dt=new DataTransfer();dt.items.add(file);input.files=dt.files;if(status)status.textContent='Restoring saved Collision data…';input.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(()=>{if(status&&/Restoring saved Collision data/.test(status.textContent||''))status.textContent='';},3500);}catch(e){console.warn('Automatic Collision restore is not supported by this browser',e);}}
