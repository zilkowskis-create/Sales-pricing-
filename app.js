...
const priceListTabs = el('priceListTabs');
...
priceListTabs?.addEventListener('click', (event) => {
  ...
  if (!tab) return;
  state.searchSettings.source = tab.dataset.priceListTab || '';
...
function getParentKey(list) {
  return list.parentFileId || `file::${list.fileName}`;
}

function getPriceListGroups() {
  const groups = new Map();
  for (const list of state.priceLists) {
    const key = getParentKey(list);
    if (!groups.has(key)) groups.set(key, { key, fileName: list.fileName, lists: [] });
    groups.get(key).lists.push(list);
  }
  return Array.from(groups.values()).sort((a,b) => a.fileName.localeCompare(b.fileName, 'en', {numeric:true, sensitivity:'base'}));
}

function currentPriceListParent() {
  const groups = getPriceListGroups();
  if (!groups.length) return '';
  const requested = state.searchSettings.source;
  if (requested && groups.some(g => g.key === requested)) return requested;
  return groups[0].key;
}

function renderPriceListTabs() {
  if (!priceListTabs) return;
  const groups = getPriceListGroups();
  if (!groups.length) { priceListTabs.innerHTML = ''; return; }

  const current = currentPriceListParent();
  if (!state.searchSettings.source || !groups.some(g => g.key === state.searchSettings.source)) {
    state.searchSettings.source = current;
    saveSearchSettings();
  }

  priceListTabs.innerHTML = groups.map(group => {
    const total = group.lists.reduce((sum, list) => sum + list.items.length, 0);
    const active = group.key === current;
    return `<button type="button" class="price-list-tab ${active ? 'active' : ''}" data-price-list-tab="${escapeAttr(group.key)}">
      <strong>${escapeHtml(shortFileName(group.fileName))}</strong>
      <small>${group.lists.length} sheets · ${total.toLocaleString('en-GB')} items</small>
    </button>`;
  }).join('');
}
...
function getSearchMatches() {
  ...
  if (state.searchSettings.filtersEnabled) {
    if (state.searchSettings.source) items = items.filter(item => { const list = state.priceLists.find(x => x.id === item.listId); return list && getParentKey(list) === state.searchSettings.source; });
...
function renderFilters() {
  ...
  const groups = getPriceListGroups();
  const sourceOptions = groups.map(group => ({ value: group.key, label: shortFileName(group.fileName) }));
  ...
  const baseItems = allSearchItems().filter(item => {
    if (!state.searchSettings.source) return true;
    const list = state.priceLists.find(x => x.id === item.listId);
    return list && getParentKey(list) === state.searchSettings.source;
  });
